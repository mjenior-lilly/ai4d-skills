...
Template for `00_Meta/VAULT-ABSTRACT.md`. It summarizes the vault's scope, content, and coverage so retrieval systems and grounding agents can decide whether to search this vault or look elsewhere.
...
4. The `scope_keywords` array in frontmatter should contain 10–20 terms optimized for retrieval matching, including domain nouns, key method names, organisms, technologies, frameworks, or other distinguishing vocabulary.
...
6. Give `Known gaps and limitations` the same weight as `Coverage strengths`. Describe missing coverage precisely so retrieval systems can reject unsuitable vaults.
...
8. **Staleness handling**:
   ...
   - A stale abstract can still guide retrieval, but do not assume it is complete.
