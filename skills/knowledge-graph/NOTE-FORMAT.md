...
Use this format for evergreen notes under `20_Permanent/`. Adapt headings only when the existing vault manifest defines a different convention. See [references/OBSIDIAN-SYNTAX.md](./references/OBSIDIAN-SYNTAX.md) for Obsidian Flavored Markdown body syntax, including embeds, callouts, heading and block references, math, and Mermaid.
...
- Frontmatter properties are typed. Obsidian recognizes text, number, checkbox (`true`/`false`), date (`YYYY-MM-DD`), date-time (`YYYY-MM-DDTHH:mm:ss`), list (YAML array), and link (`"[[Note]]"`, quoted). Use the correct type for each field, such as a number for a rating or a date for a calendar date, so Bases and Dataview can query the note.
...
- Do not record speculation, predictions, or unconfirmed hypotheticals here. A permanent note holds stable, source-grounded ground truth; log new and changing project-specific facts as timestamped, provenance-anchored notes using [PROJECT-FACT-FORMAT.md](./PROJECT-FACT-FORMAT.md) and link them from this note's `Related` section. A project fact can graduate into `Source grounding` once it is well-established.
