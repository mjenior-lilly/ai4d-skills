...
Use this format for a confirmed project-specific fact that is new or has changed: a decision, state change, or concrete project detail. Project-fact notes record **ground truth only**. Do not record speculation, predictions, proposals, or unresolved discussion; wait until the fact is confirmed. See [NOTE-FORMAT.md](./NOTE-FORMAT.md) for stable, source-grounded topic notes.
...
- **Subject resolution.** `subject` must point to an existing topic note whenever one exists; resolve it through titles, aliases, and tags. If the fact clearly concerns an implied but absent topic, create a minimal stub note or hold the fact in `10_Fleeting/` until a topic note exists. Do not force an unrelated link.
- **Typed frontmatter.** Keep `confirmed` a `date-time`, `subject` and `supersedes` quoted links, and `topics` and `tags` arrays so Bases can query the facts. See [references/BASES.md](./references/BASES.md) for a project-fact timeline view.
