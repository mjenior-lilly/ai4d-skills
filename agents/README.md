...
- `CLAUDE.md` - common cross-cutting instructions: commit-message hygiene (no attribution trailers), graceful handling of missing referenced files, pointers to the three policy files below, and an always-on directive to capture confirmed project facts into an Obsidian vault's `40_Project/` folder via the `obsidian` skill (the persistent, cross-session complement to that skill's own capture behavior).
